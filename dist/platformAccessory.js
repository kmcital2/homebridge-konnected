...
export class KonnectedPlatformAccessory {
    ...
    validSecuritySystemCurrentStates;
            ...
            case 'SecuritySystem':
                {
                    ...
                    this.platform.accessories.forEach((existingAccessory) => {
                        if (existingAccessory.context.device.triggerableModes?.includes('0')) { // stay/home
                            ...
                            this.platform.log.debug(`[${existingAccessory.displayName}] (${existingAccessory.context.device.serialNumber}) '${existingAccessory.context.device.type}' has Stay/Home Mode triggerable.`);
                        }
                        if (existingAccessory.context.device.triggerableModes?.includes('2')) { // night
                            nightModeUsed = true;
                            this.platform.log.debug(`[${existingAccessory.displayName}] (${existingAccessory.context.device.serialNumber}) '${existingAccessory.context.device.type}' has Night Mode triggerable.`);
                        }
                    });
                    if (stayHomeModeUsed) {
    ...
    async setSwitchState(value) {
        this.setAccessoryState('switch', value);
    }
    // get and set the security system states
    getSecuritySystemState(characteristic) {
        ...
        if (typeof this.accessory.context.device.state === 'undefined' ||
            ...
            this.platform.log.debug(`Assigning default state '${value}' to [${this.accessory.context.device.displayName}] (${this.accessory.context.device.serialNumber}) '${this.accessoryServiceType}' characteristic value. Awaiting zone's first state change...`);
        }
        else {
            value = this.accessory.context.device.state;
        }
        this.platform.log.debug(`Get [${this.accessory.displayName}] (${this.accessory.context.device.serialNumber}) '${this.accessory.context.device.type}' ${characteristic} characteristic value: ${this.accessory.context.device.state}`);
        return value;
    }
    setSecuritySystemState(characteristic, value) {
        this.accessory.context.device.state = value;
        this.platform.log.debug(`Set [${this.accessory.context.device.displayName}] (${this.accessory.context.device.serialNumber}) '${this.accessory.context.device.type}' ${characteristic} characteristic value: ${value}`);
        this.platform.controlSecuritySystem(value);
        return value;
    }
    // get sensor or actuator state
    getAccessoryState(type) {
        let value;
        this.platform.accessoriesRuntimeCache.forEach((runtimeCacheAccessory) => {
            if (runtimeCacheAccessory.UUID === this.accessory.context.device.UUID) {
                ...
                if ('humi' === type) {
                    ...
                    this.temperatureSensorService.updateCharacteristic(this.platform.Characteristic.CurrentTemperature, Number(temperature));
                }
                ...
                this.platform.log.debug(`Get ${logLabelDefault}[${runtimeCacheAccessory.displayName}] (${runtimeCacheAccessory.serialNumber}) as '${runtimeCacheAccessory.type}${logLabelType}' characteristic value: ${value}`);
            }
        });
        return value;
    }
    // for actuators
    setAccessoryState(type, value) {
        this.platform.accessoriesRuntimeCache.forEach((runtimeCacheAccessory) => {
            if (runtimeCacheAccessory.UUID === this.accessory.context.device.UUID) {
                ...
                this.platform.log.debug(`Set [${runtimeCacheAccessory.displayName}] (${runtimeCacheAccessory.serialNumber}) as '${runtimeCacheAccessory.type}' characteristic value: ${value}`);
            }
        });
        return value;
    }
