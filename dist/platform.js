...
import { createEspHomeClient, panelTransport } from './esphomeTransport.js';
/**
 ...
 */
export class KonnectedHomebridgePlatform {
    log;
    config;
    api;
    Service;
    ...
    buildPanelZones(panel, snapshot) {
        ...
        zones.forEach((zone) => {
            ...
            if (!type) {
                ...
                if (!inferred) {
                    this.log.warn(`[${panelLabel}] Zone '${zone.entityId}' has no 'type' and one couldn't be inferred ` +
                        `(domain '${domain}'${snap?.deviceClass ? `, device_class '${snap.deviceClass}'` : ''}). ` +
                        'Set a type explicitly.');
                    return;
                }
                type = inferred;
                this.log.info(`[${panelLabel}] Auto-typed '${zone.entityId}' as '${type}'` +
                    (snap?.deviceClass ? ` (device_class: ${snap.deviceClass}).` : ` (from domain '${domain}').`));
            }
            const accType = TYPES_TO_ACCESSORIES[type];
            if (!accType) {
                this.log.warn(`[${panelLabel}] Zone '${zone.entityId}' has an unknown type '${type}'. ` +
            ...
            const displayName = zone.name && zone.name !== '' ? zone.name : snap?.name && snap.name !== '' ? snap.name : accType[1];
            const zoneObject = {
                UUID: zoneUUID,
                displayName,
    ...
    registerSecuritySystem() {
        ...
        if (existingSecuritySystem) {
            this.log.info(`Updating existing accessory: ${existingSecuritySystem.displayName} (${existingSecuritySystem.context.device.serialNumber})`);
            this.konnectedPlatformAccessories[this.securitySystemUUID] = new KonnectedPlatformAccessory(this, existingSecuritySystem);
            ...
            this.konnectedPlatformAccessories[this.securitySystemUUID] = new KonnectedPlatformAccessory(this, newSecuritySystemAccessory);
     ...
     */
    registerAccessories(panelId, zoneObjectsArray, retainedAccessoriesArray) {
        ...
        const accessoriesToUpdateArray = [];
        const accessoriesToAddArray = [];
        zoneObjectsArray.forEach((panelZoneObject) => {
            ...
            if (existingAccessory && existingAccessory.context.device.UUID === panelZoneObject.UUID) {
                this.log.debug(`Updating existing accessory: ${existingAccessory.context.device.displayName} (${existingAccessory.context.device.serialNumber})`);
     ...
     */
    async connectPanels() {
        const panels = Array.isArray(this.config.panels) ? this.config.panels : [];
        const active = panels.filter((p) => p.host);
        if (active.length === 0) {
            this.log.warn('No panels configured. Add panels[] with a "host" and "zones" to your config.');
            return;
        }
        await Promise.all(active.map((panel) => this.connectPanel(panel).catch((error) => {
    ...
    async connectPanel(panel) {
        ...
        const collector = (entity) => snapshot.set(entity.id, entity);
        ...
        client.on('state', (entity) => this.handleEntityState(panelId, panelLabel, entity));
     ...
     */
    handleEntityState(panelId, panelLabel, entity, seed = false) {
        ...
        const serviceType = TYPES_TO_ACCESSORIES[runtimeCacheAccessory.type][0];
        ...
        this.log.debug(`[${panelLabel}] ${entity.id} -> ${entity.state} ` +
    ...
    markPanelEntitiesNoResponse(panelId) {
        this.accessoriesRuntimeCache.forEach((rca) => {
            ...
            this.konnectedPlatformAccessories[rca.UUID].service.updateCharacteristic(this.Characteristic[characteristicName], new Error('panel disconnected'));
        });
    }
    /**
     ...
     */
    processSensorAccessoryActions(accessory, defaultStateValue, resultStateValue) {
        if (defaultStateValue === resultStateValue) {
            return;
        }
        this.log.debug(`[${accessory.displayName}] (${accessory.serialNumber}) as '${accessory.type}' changed from ${defaultStateValue} to ${resultStateValue}`);
        const securitySystemAccessory = this.accessories.find((a) => a.UUID === this.securitySystemUUID);
        if (accessory.triggerableModes?.includes(String(securitySystemAccessory?.context.device.state)) &&
            ...
            this.log.info(`[${accessory.displayName}] tripped while armed; alarm will trigger in ${Math.round(this.entryTriggerDelay / 1000)}s`);
     ...
     */
    actuateAccessory(zoneUUID, value, _inboundSwitchSettings) {
        const runtimeCacheAccessory = this.accessoriesRuntimeCache.find((rca) => rca.UUID === zoneUUID);
        ...
        if (!client) {
            this.log.error(`Cannot actuate [${runtimeCacheAccessory.displayName}]: no ESPHome client for panel '${runtimeCacheAccessory.panelId}'.`);
            return;
        }
        const { entityDomain, entityObjectId } = runtimeCacheAccessory;
        this.log.debug(`Actuating [${runtimeCacheAccessory.displayName}] (${entityDomain}/${entityObjectId}) -> ${value ? 'on' : 'off'}`);
    ...
    controlSecuritySystem(value) {
        ...
        if (value < 3) {
            ...
            const audible = (typeof audibleModes !== 'undefined' && audibleModes.includes(String(value))) ||
     ...
     */
    startBeeperCountdown(durationMs) {
        this.stopBeeperCountdown();
