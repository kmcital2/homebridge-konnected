...
export class KonnectedHomebridgePlatform implements DynamicPlatformPlugin {
  ...
  private defaultTriggerableModes: string[];
  ...
  private autoDiscoverZones(panelLabel: string, snapshot: Map<string, EspHomeEntityState>, exclude: string[]): EspHomeZone[] {
    ...
    this.log.info(
      `[${panelLabel}] Auto-discovered ${discovered.length} zone(s) from the panel.` +
   ...
   */
  private mergeZones(panel: EspHomePanel, snapshot: Map<string, EspHomeEntityState>): EspHomeZone[] {
    const panelLabel = panel.name && panel.name !== '' ? panel.name : panel.host;
    const exclude = Array.isArray(panel.exclude) ? panel.exclude : [];
    const merged = new Map<string, EspHomeZone>();
  ...
  buildPanelZones(panel: EspHomePanel, snapshot: Map<string, EspHomeEntityState>) {
    ...
    zones.forEach((zone) => {
      ...
      const triggerableModes =
        zone.triggerableModes !== undefined
